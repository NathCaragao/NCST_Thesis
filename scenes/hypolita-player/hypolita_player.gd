...
@onready var hurbox_collision: CollisionShape2D = $PlayerHealthComponent/Hurtbox/HurboxCollision
...
func _flip_sprite() -> void:
	if self.playerGameData.direction > 0:
		sprite.flip_h = false
		$PlayerHealthComponent/Hurtbox/HurboxCollision.position.x = 1
	if self.playerGameData.direction < 0:
		sprite.flip_h = true
		$PlayerHealthComponent/Hurtbox/HurboxCollision.position.x = -1
